  ...
  try {
    await page.evaluate(() => {
      ...
      const setImgSrc = (img: HTMLImageElement) => {
        ...
        if (lazySrc && (!img.getAttribute("src") || img.getAttribute("src") === "#" || img.getAttribute("src") === "about:blank")) {
          img.setAttribute("src", lazySrc);
        }

        const lazySrcset = pick(img, ["data-nectar-img-srcset", "data-srcset", "data-lazy-srcset"]);
      ...
      document.querySelectorAll("picture source").forEach((el) => {
        ...
        const lazy = pick(source, ["data-srcset", "data-lazy-srcset", "data-nectar-img-srcset"]);
        if (lazy && !source.getAttribute("srcset")) {
          source.setAttribute("srcset", lazy);
        }
      });

      const bgAttrs = ["data-nectar-img-src", "data-bg-src", "data-background-image", "data-background", "data-bg"];
