...
const tryInlineLocalRelative = (value: string, basePath: string): string | null => {
  ...
  try {
    ...
    const mimeType =
      typeof guessed === 'string' && guessed ? guessed : 'application/octet-stream';
      ...
      if (node.type === 'function' && node.value === 'url') {
        ...
        if (singleFile && importDepth < 5) {
          ...
          if (asset) {
            ...
            node.nodes = [
              makeWordNode(toDataUrl(Buffer.from(rewrittenNested, 'utf8'), 'text/css')),
            ];
            return false;
          }
        }
...
const stripCodexMenuForceOpenArtifacts = ($: ReturnType<typeof load>): void => {
  const removeForcedStyleProps = (style: string): string => {
    const patterns = [
      /(?:^|;)\s*display\s*:\s*block\s*!important\s*;?/gi,
      /(?:^|;)\s*visibility\s*:\s*visible\s*!important\s*;?/gi,
      /(?:^|;)\s*opacity\s*:\s*1\s*!important\s*;?/gi,
      /(?:^|;)\s*pointer-events\s*:\s*auto\s*!important\s*;?/gi,
      /(?:^|;)\s*max-height\s*:\s*none\s*!important\s*;?/gi,
      /(?:^|;)\s*height\s*:\s*auto\s*!important\s*;?/gi,
      /(?:^|;)\s*transform\s*:\s*none\s*!important\s*;?/gi,
      /(?:^|;)\s*clip\s*:\s*auto\s*!important\s*;?/gi,
      /(?:^|;)\s*overflow\s*:\s*visible\s*!important\s*;?/gi,
    ];
    let out = style;
  ...
  $('style').each((_, element) => {
    ...
    if (css.includes('.codex-menu-force-open')) {
      $(element).remove();
    }
  });

  $('.codex-menu-force-open').each((_, element) => {
    const el = $(element);
    el.removeClass('codex-menu-force-open');

    const style = el.attr('style');
    if (style) {
      const cleaned = removeForcedStyleProps(style);
    ...
    if (el.attr('aria-expanded') === 'true') {
      ...
      const hasPopup = el.attr('aria-haspopup') !== undefined;
      const isMenuControl =
        className.includes('menu-toggle') ||
        className.includes('dropdown-icon') ||
...
export const rewriteHtml = (
  ...
  const shouldInlineAttr = (element: any, attr: string, resolvedUrl: string): boolean => {
    ...
    if (tag === 'script' && attr === 'src') return false;
    ...
    const rewritten = srcset
      ...
      .map((candidate) => {
        ...
        if (singleFile) {
          ...
          if (inlinedLocal)
            return descriptor ? `${inlinedLocal} ${descriptor}` : inlinedLocal;
        }
        const resolved = resolveUrl(urlPart, pageUrl);
  ...
  $('[style]').each((_, element) => {
    ...
    if (!value) return;
    try {
      const rewritten = rewriteInlineStyleValue(
        value,
        pageUrl,
        pagePath,
        resourceMap,
  ...
  backgroundDataAttrs.forEach((attr) => {
    ...
    $(selector).each((_, element) => {
      ...
      const bgRule = hasBg
        ? ''
        : `background-image: url(${value}); background-size: cover; background-position: 50% 50%; background-repeat: no-repeat; `;
